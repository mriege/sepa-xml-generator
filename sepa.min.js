...
(function (t) {
  ...
  /**
   * Prueft, ob ein pain-Format auf den ISO-20022-Basistypen von 2019 aufsetzt.
   *
   * Beide Nachrichtenfamilien haben mit der Endung ".08" umgestellt:
   *   pain.001.001.03 / pain.008.001.02  -> alte Basistypen (2009)
   *   pain.001.001.08 / .09              -> neue Basistypen (2019)
   *   pain.008.001.08                    -> neue Basistypen (2019)
   *
   * Bewusst NICHT ueber l() ermittelt: l() addiert bei pain.008 eine 1 und
   * vermischt dadurch die beiden Familien (pain.001.001.03 und pain.008.001.02
   * ergeben beide 3). Fuer die Basistyp-Frage ist nur die letzte Zahl relevant.
   *
   * @param {string} t - Pain-Format-String (z.B. "pain.008.001.08")
   * @returns {boolean} true bei .08 und neuer
   */
  function usesIso2019Types(t) {
    return parseInt(t.substr(-2), 10) >= 8
  }

  /**
   * Liefert den XML-Tag-Namen fuer den BIC innerhalb von <FinInstnId>.
   *
   * Die 2019er-Basistypen (FinancialInstitutionIdentification18) benennen das
   * Element <BICFI>; die aelteren (FinancialInstitutionIdentification7/8)
   * <BIC>. Ein pauschales Ersetzen wuerde die weiterhin verbreiteten Formate
   * pain.008.001.02 / pain.001.001.03 ungueltig machen – daher formatabhaengig.
   *
   * Ohne diese Unterscheidung lehnen Banken .08/.09-Dateien ab mit
   * "Element 'BIC' is not expected. Expected is one of ( BICFI, Othr )".
   *
   * @param {string} t - Pain-Format-String
   * @returns {string} "BICFI" oder "BIC"
   */
  function bicTagName(t) {
    return usesIso2019Types(t) ? "BICFI" : "BIC"
  }
  ...
  /**
   * Erlaubter Zeichenvorrat fuer Freitextfelder nach EPC217-08:
   *   a-z A-Z 0-9 Leerzeichen und  / - ? : ( ) . , ' +
   * Alles andere wird von vielen Bankprogrammen abgelehnt oder still
   * verstuemmelt – auch bei korrekter UTF-8-Kodierung der Datei.
   */
  var SEPA_ALLOWED = /[^A-Za-z0-9\/\-?:().,'+ ]/g;

  /**
   * Ersetzungen fuer Zeichen, bei denen ein reines Weglassen den Text
   * unleserlich machen wuerde. Umlaute werden nach deutscher Konvention
   * aufgeloest (ae/oe/ue/ss), nicht auf den Grundbuchstaben reduziert.
   */
  var SEPA_REPLACEMENTS = {
  ...
  /**
   * Wandelt beliebigen Text in den SEPA-Zeichensatz um.
   *
   * Ablauf (die Reihenfolge ist wesentlich):
   *   1. Zeichenweise Ersetzung ueber SEPA_REPLACEMENTS (ae/oe/ue/ss, & -> +)
   *   2. Unicode-Zerlegung (NFD) und Entfernen der Kombinationszeichen,
   *      damit auch é/à/ç/ñ zu e/a/c/n werden. Muss NACH Schritt 1 laufen:
   *      in NFD zerfaellt "ä" zu "a" + Trema und wuerde sonst zu "a" statt "ae".
   *   3. Alles noch Verbliebene ausserhalb des Zeichensatzes zu Leerzeichen,
   *      Mehrfach-Leerzeichen zusammenfassen, trimmen.
   *   4. Erst danach auf die Feldlaenge kuerzen – "ß" -> "ss" verlaengert den
   *      Text, ein Kuerzen davor wuerde die Laengengrenze wieder reissen.
   *
   * @param {*} t - Eingabewert (auch Zahlen aus Excel-Importen zulaessig)
   * @param {number} [n] - Maximale Feldlaenge (z.B. 70 fuer Nm, 140 fuer Ustrd)
   * @returns {string} Text im SEPA-Zeichensatz
   */
  function sanitizeSepaText(t, n) {
    ...
    for (var o = 0; o < e.length; ++o) {
      var f = e.charAt(o);
      r += Object.prototype.hasOwnProperty.call(SEPA_REPLACEMENTS, f) ? SEPA_REPLACEMENTS[f] : f
    }
    if (typeof r.normalize === "function") {
      r = r.normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    }
    r = r.replace(SEPA_ALLOWED, " ").replace(/ {2,}/g, " ").replace(/^ +| +$/g, "");
    if (n && r.length > n) {
      r = r.substr(0, n).replace(/ +$/, "")
    }
    return r
  }
  ...
  h.prototype = {
    ...
    toString: function () {
      var t = this.toXML();
      // Interne Werte statt t.xmlVersion/t.xmlEncoding verwenden: Im Browser
      // sind diese DOM-Properties read-only (bzw. aus dem Standard entfernt),
      // die Zuweisungen in toXML() greifen nicht und Lesen liefert null.
      // Ergebnis war '<?xml version="null" encoding="null"?>' – Banken lehnen
      // die invalide Deklaration mit "Dateiformat wird nicht unterstützt" ab.
      var n = '<?xml version="' + this._xmlVersion + '" encoding="' + this._xmlEncoding + '"?>';
      return n + L(t)
    }
  };
  ...
  m.prototype = {
    ...
    toXML: function (t) {
      ...
      // Erstellungszeitpunkt ohne Millisekunden: xs:dateTime erlaubt zwar
      // Sekundenbruchteile, mehrere Bankprogramme erwarten aber die von der
      // DK/EPC dokumentierte Form YYYY-MM-DDThh:mm:ss und lehnen ".000Z" ab.
      n(e, "CreDtTm", this.created.toISOString().replace(/\.\d{3}Z$/, "Z"));
      if (r === 2) {
  ...
  p.prototype = {
    ...
    localInstrumentation: "CORE",      // LclInstrm/Cd - CORE oder B2B
     ...
     *   - localInstrumentation: Muss CORE oder B2B sein
    ...
    validate: function () {
      ...
      // Lastschrift-spezifische Validierungen
      if (this.method === f.DirectDebit) {
        // COR1 ("Eil-Lastschrift", D-1) wurde zum 21.11.2016 abgeschafft: seither
        // gilt die verkuerzte Vorlaufzeit fuer CORE, und CORE hat die Funktion
        // uebernommen. Die Schemata der Kreditwirtschaft kennen den Code nicht
        // mehr und lehnen ab mit "Element 'Cd': [facet 'enumeration'] The value
        // 'COR1' is not an element of the set".
        B(this.localInstrumentation, ["CORE", "B2B"], "localInstrumentation");
    ...
    toXML: function (t) {
      ...
      // Ab Version 3 (pain.008.001.02, pain.001.001.03 und alle neueren wie
      // .08/.09): BatchBooking, Anzahl und Summe im PmtInf. Die deutsche
      // Kreditwirtschaft verlangt NbOfTxs/CtrlSum hier zwingend – mit "=== 3"
      // fehlten sie bei .08/.09 und Banken lehnten ab mit "Das Tag 'PmtTpInf'
      // wird an dieser Stelle nicht erwartet. Stattdessen wird 'NbOfTxs' erwartet."
      ...
      if (this.method === f.DirectDebit) {
        ...
        e(i, "LclInstrm", "Cd", this.localInstrumentation);  // CORE oder B2B
        ...
        // Ab pain.001.001.08 ist ReqdExctnDt keine ISODate mehr, sondern ein
        // DateAndDateTime2Choice – der Wert muss in <Dt> verschachtelt werden.
        // Flach geschrieben lehnen Banken ab mit "Element 'ReqdExctnDt':
        // Missing child element(s). Expected is one of ( Dt, DtTm )".
        var execDt = this.requestedExecutionDate.toISOString().substr(0, 10);
        if (usesIso2019Types(this._painFormat)) {
          e(r, "ReqdExctnDt", "Dt", execDt)
        } else {
          e(r, "ReqdExctnDt", execDt)
        }
      }
      ...
      // Bank (BIC/BICFI) - wenn vorhanden, sonst "NOTPROVIDED"
      if (this[o + "BIC"]) {
        e(r, a + "Agt", "FinInstnId", bicTagName(this._painFormat), this[o + "BIC"])
      } else {
  ...
  y.prototype = {
    ...
    toXML: function (t) {
      ...
      // EndToEndId ist Pflicht und darf nicht leer sein (minLength 1). Ohne
      // Fallback entstand <EndToEndId/> und Banken lehnten ab mit "Datei
      // enthaelt folgenden invaliden Wert: ". "NOTPROVIDED" ist der dafuer
      // vorgesehene Platzhalter der EPC-Implementierungsrichtlinien.
      o(s, "EndToEndId", this.end2endId || "NOTPROVIDED");
      ...
      if (this[n + "BIC"]) {
        o(a, e + "Agt", "FinInstnId", bicTagName(this._painFormat), this[n + "BIC"])
      } else {
      ...
      // Verwendungszweck (unstrukturiert) – nur erzeugen wenn vorhanden:
      // RmtInf ist optional, aber ein leeres <Ustrd/> verletzt das Schema
      // (minLength 1) und Banken lehnen ab mit "invalider Wert".
      i(a, "RmtInf", "Ustrd", this.remittanceInfo);
   ...
   */
  function w(t, n) {
    // Excel-Importe liefern IDs teils als Number (z.B. Referenz 12345) –
    // ohne String-Koersion wirft t.match "is not a function".
   ...
   */
  function F(t, n) {
    // Excel-Importe liefern Mandatsreferenzen teils als Number (z.B. 12345) –
    // ohne String-Koersion wirft t.match "is not a function".
    if (typeof t === "number") t = String(t);
    if (t && !t.match(/([A-Za-z0-9]|[+|?|/|\-|:|(|)|.|,|']){1,35}/)) {
  ...
  t.toSepaText = sanitizeSepaText;  // SEPA.toSepaText(text, maxLen) → string
  t.bicTagName = bicTagName         // SEPA.bicTagName(painFormat) → "BIC"|"BICFI"
